...
async function startTask(taskId) {
  lastError.value = '';
  const task = store.appState.reminderTasks.find((t) => t.id === taskId);
  if (task) {
    startCountdownTask(task);
    await store.persistReminderTasks();
  }
}
</script>
...
<template>
  <div class="countdown-panel">
    ...
    <section class="list-section">
      ...
      <p v-if="!store.appState.reminderTasks.length" class="muted">暂无任务，先创建一个。</p>
      <ul v-else class="list">
        <li v-for="t in store.appState.reminderTasks" :key="t.id" class="item">
          <div class="meta">
